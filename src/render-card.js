...
const renderCard = (cardData) => {
  ...
  ${(!cardData.hasControls) ? `` : `<form class="film-card__controls"><button class="film-card__controls-item button film-card__controls-item--add-to-watchlist">${cardData.inWatchlist ? `WL` : `Add to watchlist`}</button><button class="film-card__controls-item button film-card__controls-item--mark-as-watched">${cardData.isWatched ? `WTCHD` : `Mark as watched`}</button><button class="film-card__controls-item button film-card__controls-item--favorite">${cardData.isFavorite ? `FAV` : `Mark as favorite`}</button></form>`}
  </article>`;
